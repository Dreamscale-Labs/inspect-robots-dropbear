...
# The rate this embodiment actually steps at, halved from the native 30 Hz.
# The 24-action chunk is stretched in time rather than resampled, so it now
# spans 1.6 s instead of 0.8 s -- every action still executes, just slower.
# This must equal the policy's `control_hz`; see run_eval.py.
#
